...
def generate_image_sizes()
	...
	imagemagick_command = "find . -maxdepth 1 -type f  \\( -name '*.tif' -o -name '*.png' -o -name '*.jpg' -o -name '*.jpeg' \\) | xargs -P 4 -I@  #{IMAGEMAGICK_COMMAND} @ -quality #{WEBP_QUALITY} -set filename:name '%t' '#{DESTINATION_DIRECTORY_NAME}/%[filename:name].#{OUTPUT_FILE_EXTENSION}'"
